...
while true; do
	for disk in "$@"; do
		...
		eval `sudo /usr/sbin/smartctl -B /usr/local/share/smartmontools/drivedb.h -n standby $drv -A "/dev/$dsk" | awk '$3 ~ /^0x/ && $2 ~ /^[a-zA-Z0-9_-]+$/ { gsub(/-/, "_"); print "SMART_" $2 "=" $10 }' 2>/dev/null`
